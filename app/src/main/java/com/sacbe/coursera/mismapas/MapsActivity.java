...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        foto = parametros.getInt(getResources().getString(R.string.pFoto));
        marcador = parametros.getInt(getResources().getString(R.string.pMarcador));
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        //mMap.addMarker(new MarkerOptions().position(mexico).title(nombre));
        //mMap.moveCamera(CameraUpdateFactory.newLatLng(mexico));

        mMap.addMarker(new MarkerOptions()
                .icon(BitmapDescriptorFactory.fromResource(marcador))
                .anchor(0.0f, -0.1f) // Anchors the marker on the bottom left
                .position(new LatLng(latitud, longitud))
                .title(nombre));

        mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(
                new LatLng(latitud, longitud), 16));
    }
}
